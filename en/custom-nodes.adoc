...
Here `MyEnum` is what we call the type and `{ Foo, Bar }` makes its definition. In order to create a dynamic enum for VL we also need those two elements. Both need to be implemented as classes in c#:
