...
. Unfolding it you should see the 'CreateDefault' operation which you can drag-drop onto the document canvas. The name 'CreateDefault' will hint the type-system to use this operation to initialize the type, whenever it encounters the type `MyEnum`.
