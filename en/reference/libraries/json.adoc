...
[NOTE]
====
If in an advanced use case you find yourself in a situation where this conversion from JSON to XElement is not feasible, you can still operate on JSON directly by referencing a library like https://www.newtonsoft.com/json[JSON.NET], as described in link:using-net-libraries.adoc[Using .NET Libraries].
====
