...
== Helper Patch
Patch which defines one or more Operations. Can not contain Properties

== Class Patch
Patch which defines a datatype. Contains at least a _create_ and an _update_ operation and may contain properties 
...
== Node
Visual element in a Patch which represents the call of an Operation or instance of a Class Patch
...
== Operation Node
Node which calls an Operation

== Instance Node
Node which represents a fixed instance of a Class Patch

== Instance Operation Node
Node which calls an Operation that is part of a Class Patch
