...
[Installer win-x64](https://teamcity.vvvv.org/guestAuth/app/rest/builds/id:40132/artifacts/content/vvvv_gamma_7.4-win-x64_setup.exe) | [Offline Installer win-x64](https://teamcity.vvvv.org/guestAuth/app/rest/builds/id:40132/artifacts/content/vvvv_gamma_7.4-win-x64_setup_offline.exe)  
[Installer win-arm64](https://teamcity.vvvv.org/guestAuth/app/rest/builds/id:40133/artifacts/content/vvvv_gamma_7.4-win-arm64_setup.exe) | [Offline Installer win-arm64](https://teamcity.vvvv.org/guestAuth/app/rest/builds/id:40133/artifacts/content/vvvv_gamma_7.4-win-arm64_setup_offline.exe)

#### Fixes
- Fixes issue with VisualStudio opening when creating a new shader, but failing to load the project
- Fixes regression introduced in 7.2 that GetFiles with empty string was no longer interpreted as wildcard
- Rotation (Successive) has its "Local" input renamed to "Global" to correspond naming to actual functionality plus helppatch added

### Stride
- Fixes resource leak when changing params on DepthOfField - [forum](https://forum.vvvv.org/t/depth-of-field-postfx-leaks-memory-on-area-change/25171)

## vvvv gamma 7.3
released on 24 06 2026  
[Installer win-x64](https://teamcity.vvvv.org/guestAuth/app/rest/builds/id:40082/artifacts/content/vvvv_gamma_7.3-win-x64_setup.exe) | [Offline Installer win-x64](https://teamcity.vvvv.org/guestAuth/app/rest/builds/id:40082/artifacts/content/vvvv_gamma_7.3-win-x64_setup_offline.exe)  
[Installer win-arm64](https://teamcity.vvvv.org/guestAuth/app/rest/builds/id:40083/artifacts/content/vvvv_gamma_7.3-win-arm64_setup.exe) | [Offline Installer win-arm64](https://teamcity.vvvv.org/guestAuth/app/rest/builds/id:40083/artifacts/content/vvvv_gamma_7.3-win-arm64_setup_offline.exe)

#### New
- Now shows warning before reloading a document with unsaved changes
- Fragment attribute in C# now recognized on getter or setter method as well. Allow for more fine-grained process definitions.

#### Fixes
- Fixes FrameFinishedMessage.LastInterval which in turn fixes Stride Physics - regression from 7.2 - [forum](https://forum.vvvv.org/t/stride-physics-seems-to-be-broken-in-version-7-2/25139)
- Fixes newly introduced null pointer exception after inserting pad into link via double right click - regression from 7.2
- Fixes exported app not shutting down correctly - regression from 7.2

### Standard Libraries
- Adds Homography (2D DoubleQuad), QuadRange and DoubleQuadRange nodes
- Fixes TextureToSkImage not working for typeless formats
- Fixes colorspace conversion issues between Stride and Skia. Note that `VideoPlayer` and `VideoIn` now use linear texture formats when running on GPU. Before they had to make an internal texture copy in order to convert from BGRA to BGRA_Srgb, now they simply use a Half format to start with. So overall should lead to same memory usage but one less copy operation.
- Skia `CallerInfo` gives access to `SKSurface`

### Stride
- `DirectDispatcher` node will emit warning in case the count exceeds maximum

### Redis
- `RecisClient` node has more options to configure client side caching. It's now possible to select the [default caching mode](https://redis.io/docs/latest/develop/reference/client-side-caching/#the-redis-implementation-of-client-side-caching) in which the server keeps track of all keys read by client and for the [broadcast mode](https://redis.io/docs/latest/develop/reference/client-side-caching/#broadcasting-mode) (which we use by default) one can now set the prefixes.
- Adds optional pin to advanced nodes which allows to override the database one wants to operate on

## vvvv gamma 7.2
released on 22 05 2026  
[Installer win-x64](https://teamcity.vvvv.org/guestAuth/app/rest/builds/id:40001/artifacts/content/vvvv_gamma_7.2-win-x64_setup.exe) | [Offline Installer win-x64](https://teamcity.vvvv.org/guestAuth/app/rest/builds/id:40001/artifacts/content/vvvv_gamma_7.2-win-x64_setup_offline.exe)  
[Installer win-arm64](https://teamcity.vvvv.org/guestAuth/app/rest/builds/id:40002/artifacts/content/vvvv_gamma_7.2-win-arm64_setup.exe) | [Offline Installer win-arm64](https://teamcity.vvvv.org/guestAuth/app/rest/builds/id:40002/artifacts/content/vvvv_gamma_7.2-win-arm64_setup_offline.exe)

### vvvv itself - HDE & Runtime

#### New
- New attribute `SkipCategoryAttribute` useful on static helper classes where we want to skip the class from opening a category
- New attribute `NameAttribute` - allows to name symbols differently in VL than in dotnet - [forum](https://forum.vvvv.org/t/generic-interface-with-unigeneric-version-importasis/25075)

#### Changes
- Adds new setting to switch patch editor to WARP device. This new setting is primarily for debugging purposes. Certain tooltips relying on shared textures (images, scenes, etc.) will no longer work.
- Adds option to run user patch in main thread. Again, primarily for debugging purposes.
- Better logging when stopping patches and added debug logging for various internal compiler steps.
- CollectionViewer now operating on `IReadOnlyCollection<T>` which is implemented by far more types than former `ICollection` (like for example `ArraySegment` used in VL.Buffers)

#### Fixes
- Fixes region code generator not emitting needed state classes of nested regions
- Fixes incorrect target code getting generated when connecting Foo to Bar with `Foo : Bar<Some>` and `Bar<T> : IChannel<T>` - [forum](https://forum.vvvv.org/t/weird-crash-on-inheritance-chain-cs1503/25093/2)
- Fixes enum dropdown showing double text entries for enums where entries map to the same value
- Fixes C# written process nodes not inheriting "Aspects" of parent category
- Fixes "new version available" button not showing up (was broken since 7.0)

### Standard Libraries

- More efficient TryGetValueByPath implementation
- IO.Path fixes an inconsistency in Equals & GetHashCode implementation ("c:\foo\" now is different from "c:\foo")
- Better handling when switching sources / disabling sources connected to VideoSourceTo* nodes. The previous version did not release the latest received video frame when switching the source which could lead to upstream pipelines to stay open / connected as long as they didn't produce a new frame.
- New packages `VL.Buffers` centered around the `System.Buffers` namespace
- Fixes Path.CopyTo not working for single file with "Replace Existing == true"

#### Stride
- New FlatCameraRenderer and OrbitCameraRenderer nodes
- QuadRenderer with new inputs: Texture Transform, Sampler and Color input is now of type `GPU<RGBA>`
- Group, MeshRenderer and all renderers in Stride.Models.Meshes now have an "Enabled" input
- Adds new "Is Borderless" pin to Scene- and RenderWindow to be able to setup window correctly right from the start
- Fixes white (or black) flash on startup because the window is only made visible after the frame has been rendered and not up front
- Fixes commands sent to editor executing immediately inside of Update call which in turn could cause crash because a Stop command would dispose the state
- Fixes VideoSourceToTexture using 100% CPU in case source hands out null frames

#### Redis
- Adds IsDisposed check to Subscribe node to ensure it doesn't prevent shutdown
- Saves a boxing allocation in a core method, should lead to less allocation in Binding node
- Fixes handling of strings when using serialization `Raw`
...
- Fixes active tab jumping to "random" tab when closing another - [forum](https://forum.vvvv.org/t/the-issue-with-jumping-when-closing-a-tab/25044)
