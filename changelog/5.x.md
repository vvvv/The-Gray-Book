...
## Changes
* Channels got more versatile. Every `Channel<T>` now can be cast to a `Channel<Object` (and back if you know `T`). The `ChannelOfObject`now is implemented by the same channel increasing performance for those cases where you talk to channels in a non-generic way.
...
* Introducing Global Channels
  ...
  * A `GlobalChannel` node give dynamic access to them. You feed the Channel Path at runtime
...
* Swappable objects renew their state on the fly when accessed via VLPropertyInfo.GetValue.
