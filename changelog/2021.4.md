...
## New 
* Compile local patch: press F4. In an ideal world this is not necessary as the compiler recompiles every patch that is affected indirectly by any of your changes. In the real world however you can press F4 if this didn't happen. 
...
## NodeBrowser
* NodeBrowser now offering overloads instead of pin choices
* Single pin choice mode now done in a way that we don't need the OK or Cancel buttons (the assumption here is that in this mode you always want the shortest node and you'd open up the node browser again if you miss a pin)
