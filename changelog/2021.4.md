...
* Hotswap now is much more reliable and is covering more cases that can be hotswapped. Even complex cases are taken care of, where we need to rewire self-referential object graphs after hotswapping parts of it. But the most important part is: we can guarantee that no outdated code will be able to run. In other words: if the hotswap was too complex and wasn't done quite right, we detect that and restart just in time so you'll alsways see the latest patch running and not some old version of it.
* More robust runtime
