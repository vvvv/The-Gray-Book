...
* Skia: major internal paint and layer releated rework leading to performance improvements, less memory usage and a more stable frame rate.
* Skia: added SetTextSize (InPixels) node
* Skia: Group nodes now have a debug feature
* Skia Renderer: added Enable Keyboard Shortcuts pin 
* Skia Renderer: added SetBounds operation with some options (in pixels/DIPs, netto/brutto size)
* Skia: added experimental RenderWindow (Upcoming) allowing to specify the Bounds with a regular Rectangle 
