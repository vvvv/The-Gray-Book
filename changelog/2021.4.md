...
* No longer generate proxy classes for records - they shall be as lightweight as possible. Reasoning is that we don't patch inside of records that often. Since they usually pop-up as leaves inside of object graphs, the performance impact of the proxy indirection layer can be quite severe.
...
### Skia
* major internal paint and layer releated rework leading to performance improvements, less memory usage and a more stable frame rate.
...
* Offscreen renderer will throw exception if called from wrong thread
