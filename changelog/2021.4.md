...
[Offline Installer](https://teamcity.vvvv.org/repository/download/vvvv_gamma_Build/36569:id/vvvv_gamma_2021.4.11_setup_offline.exe)

## New
* You can now [play Lottie files](https://visualprogramming.net/blog/2022/introducing-lottie/) with Skia 
* You can now [share screenshots](https://visualprogramming.net/blog/2022/introducing-show-tell) of your work to our [collective gallery](https://pixelfed.social/madewithvvvv)
* You can now take screenshots of Skia and Stride renderers pressing Ctrl+3
* New command line option to specify the primary package folder via `--nuget-path` - [forum](https://discourse.vvvv.org/t/vvvv-projects-self-contained-vs-global-package-repositories/20754/10)
...
## Fixes
* Thanks to a new hotswap friendly target code, pressure on the garbage collector is reduced drastically, fixing lot's of glitches/dropouts while running inside of the patch editor. Exported apps are not affected by this change - [forum](https://discourse.vvvv.org/t/total-frame-drop-out-including-audio-dropout-glitching/20701/11)
* Hotswap now is much more reliable and is covering more cases. Even complex cases are taken care of, where we need to rewire self-referential object graphs after hotswapping parts. But most importantly we now can guarantee that no outdated code will be able to run even in failed hoswap cases as we have a safety net installed that is able to detect outdated objects and restarts if necessary.
* More robust runtime
* Compiler bug fixes regarding CustomRegion API
* Fixing infamous "X is no X" errors
* Compiler bug fix: in rare cases compiler tried to convert T to Monadic of T, where it wasn't assignment compatible.
...
* Fixes `ReleaseAndDispose` mode of `SingleInstance` region not working in exported apps - [forum](https://discourse.vvvv.org/t/exported-application-hangs-on-exit-when-using-vl-io-mousekeyglobal/20840)
* Fixes & documents `StoreSequence` node
* Fixes exception when creating a dictionary pin group and an internal equality check leading to endless compile loop - [forum](https://discourse.vvvv.org/t/dynamic-inputs-cause-gamma-to-crash-in-a-very-strange-way/20748/3)
...
* Fixes seek time not being passed to image player when running without time sync - [forum](https://discourse.vvvv.org/t/imageplayer-stride-doesnt-preload-correctly/20956)
* Fixes regions created with the `Surround` command not being replaceable - [forum](https://discourse.vvvv.org/t/its-not-possible-to-replace-regions-that-have-been-created-with-surround/20930/2)
...
* Fixes vvvv.exe not properly shutting down - [forum](https://discourse.vvvv.org/t/iresourceprovider-seems-not-to-dispose-resource/20676/3)
