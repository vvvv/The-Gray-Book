...
* Node Browser that searches for connectable nodes: 
	...
	* Choice comes with full category path -> [fixes TransformSRT issue](https://discourse.vvvv.org/t/seaching-for-srt-when-connected-to-a-matrix-input-only-gives-2d-transform/22365)
...
- ABI stability fix for VL.Scalable
...
- Fixes non-deterministic behavior of some stride material nodes. Copy on write didn't work as Stride didn't implement equality for their types properly (e.g. for MaterialSpecularThinGlassModelFeature). Using ReferenceEquality now.
