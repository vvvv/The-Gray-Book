...
### Changes
* Changing values in pins and pads no longer needs a compile step. This improves playfulness.
* Drawing links is now easier to get right when starting from an IOBox, Pad or Control point. [Reported here](https://discourse.vvvv.org/t/beginners-struggle-with-starting-a-link-from-the-bottom-of-an-iobox/21566)
* Patch Editor & Node Browser: Improved patching speed. _TODO: add a link to a blog post for the details_
	* Better support for touchpads and tablets
	...
	* Drawing a link also works with drag & drop gesture. Dropping works on region bars, existing data hubs, empty patch.
	* Node browser double open when doing "unprecise" double click
	* For those that don't like middle-clicks: to create an IOBox at the end of a link: double-click, choose IOBox, get the IOBox correctly typed (so one double click and a click instead of one middle-click)
* Refined auto link segmentation. Sometimes you ended up with only ugly links, sometimes with a completely unreadable patch. Cursor helps you when drawing links by showing which patch areas you can link into. Setting now allows to disable link auto segmentation
...
* Node Browser that searches for connectable nodes: 
	* Faster and more relevant results at the top. Examples:
		* Starting from a input pin of type `GPU<RGBA>` and link upstream: nodes outputting RGBA come directly after those that output `GPU<RGBA>`
		* Starting from RootScene and link downstream: We see SceneWindow, SceneTexture
	* Choice comes with full category path -> TransformSRT [3D.Transform] can now be created. [fixes TransformSRT issue](https://discourse.vvvv.org/t/seaching-for-srt-when-connected-to-a-matrix-input-only-gives-2d-transform/22365)
	* Categories in choices can be searched for
	* Rich-text choices: greying out category for better readability
	* Node browser allows to go in and out-of connectable-nodes mode on the fly.
...
* Warning on through-pads in records as proposed [here](https://github.com/vvvv/VL-Language/issues/42)
* Node browser: 
	* Now draws itself via Skia - was System.Drawing
	* IOBox creation: Int2/3/4 supported
	* Pressing "." when drawing a link opens node browser filtered for the type 
	* Filtering via type also possible when creating the node while linking. The type of the link is choosable as one of the first choices
	...
	* When entering "pa" pad shows up again, even when not in new mode. This aligns with the fact that you don't have to click into categories in order to see all nodes that start with "pa"
	* Primitive nodes show first. entering "If" or "Ca" will show "If" or "Cache" before any type.
	* Sorting in general got improved
...
- Thread-safe adaptive node witness building. fixes: https://github.com/vvvv/VL.StandardLibs/issues/642
- Compiler Fix for a corrupt patch with declaring a Property twice. Now reports the error.
- IOBox Editors: nested editors (e.g. Spread of Enum) move to front. Fixes https://discourse.vvvv.org/t/dropdowns-outside-regions-get-occluded-by-elements-inside-regions/21681
- Don't show "Default: " on pin views of node application when pin is unused. This tricks people into thinking that this default is somehow relevant, which it is not, since the fragment of that pin doesn't get executed anyway. https://discourse.vvvv.org/t/display-of-pins-on-process-nodes/21912
...
- Nodebrowser - creating iobox. pressing ESCAPE doesn't close the nodebrowser but goes back into main mode
...
- Moment tinting in regions wasn't quite right. Could lead to a node showing up in grey while actually being executed on Create.
- Fix for dark theme: Unused pins were just hard to distinguish from normal pins.
- Disables feature "auto-convert a string-IOBox into a link-IOBox if string is a URL) - was perceives as a bug
- NodeBrowser: temp node info on the right panel wasn't going away when it should
- When linking and creating an iobox upstream make sure that a spread iobox with one entry gets created if sink is uninitialized. If sink is having a value take this.
...
- Send & Receive nodes refined
	- `S` & `R` nodes now a re called `Send (Global)` & `Receive (Global)` 
	- `Send (Local)` region + `R (Local)` node got added for a better support of scopes. Values are only receivable in a defined area of the patch. Reasoning about execution order not necessary.
...
- ObjectEditor: fix for deeply mutating object structure. Now displaying the changes deep inside.
